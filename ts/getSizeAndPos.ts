...
export const getSizeAndPos = (options: Options, winKey: WindowID, displayBounds: IBounds) => ({
  left: Math.round(
    options.get(getStorageWindowPropKey(winKey, "left")) * displayBounds.width + displayBounds.left,
  ),
  top: Math.round(
    options.get(getStorageWindowPropKey(winKey, "top")) * displayBounds.height + displayBounds.top,
  ),
